...
bool validateRequest(DynamicJsonDocument request)
{
	...
	if (validateReadAction(request["action"]))
	{
		if (!validateInputPin((int)request["pin"]))
		{
			Serial.write("{\"response\": 1, \"msg\": \"Pin is not valid\"}\r\n");
			return false;
		}
	}
	else if (!validatePin((int)request["pin"]))
	{
		Serial.write("{\"response\": 1, \"msg\": \"Pin is not valid\"}\r\n");
		return false;
	}
...
{
	if (action == ACTION_READ || action == ACTION_GET)
	{
		return true;
	}
	return false;
...
bool validateAction(String action)
{
	if (action == ACTION_ON || action == ACTION_OFF || action == ACTION_SET_MODE ||
		action == ACTION_READ || action == ACTION_GET)
	{
		return true;
	}
	return false;
...
bool validateReadType(String type)
{
	if (type == READ_TYPE_DHT)
	{
		return true;
	}
	return false;
